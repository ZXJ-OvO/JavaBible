...
`JDK`：即Java Development Kit，是提供给开发者使用的Java SDK，能够创建和编译 Java 程序。他包含了 JRE 和 一些开发工具

`JVM`：即Java Virtual Machine，是运行 Java 字节码的虚拟机。JVM针对不同的系统有不同的实现，目的是实现相同的字节码输出相同的结果。字节码和不同平台的JVM实现是Java实现一次编译随处运行的关键所在

> JVM不止一种，比如我们常见的HotSpot还有其他很多种，只要满足 JVM 规范，每个公司、组织或者个人都可以开发自己的专属 JVM

`JRE`：即Java Runtime Environment， 是 Java 运行时环境。它是运行已编译 Java 程序所需的所有内容的集合，主要包括 Java 虚拟机（JVM）、Java 基础类库

> 如果只需要运行Java程序，只需要安装JRE，如果需要开发Java程序则需要安装JDK，但是如果运行的程序需要使用Java的一些特性，如 JSP 转换为 Java Servlet、使用反射等，那么光有JRE还是不够的，仍然需要安装JDK
...
- 可以看到，像 `byte`、`short`、`int`、`long`能表示的最大正数都减 1 。这是因为在二进制补码表示法中，最高位是用来表示符号的（0 表示正数，1 表示负数），其余位表示数值部分。所以，如果我们要表示最大的正数，我们需要把除了最高位之外的所有位都设为 1。如果我们再加 1，就会导致溢出，变成一个负数。
- 对于 `boolean`，官方文档未明确定义，它依赖于 JVM 厂商的具体实现。逻辑上理解是占用 1 位，但是实际中会考虑计算机高效存储因素。
- 另外，Java 的基本类型所占存储空间不会像其他语言那样随硬件变化而变化。这种不变性是 Java 程序比其他程序具有更强的可移植性的原因之一
...
- 因为Java只有值传递，所以不管是`基本数据类型`还是`引用数据类型`，`==`比较的都是值，只不过引用类型存的值是对象的地址。

**`equals()`**

- 比较基本数据类型会抛检查时异常，只能用来判断两个对象是否相等。
...
- 数据库：（DataBase、DB）数据库是由数据库管理系统管理的数据的集合。
- 数据库管理系统：（Database Management System、DBMS）是一种操作和管理数据库的大型软件
- 数据库系统：（Data Base System、DBS）通常由软件、数据库和数据管理员（DBA）组成
...
- 元组：（行）关系数据库中的基本概念，类似于现实生活中的一条记录或者一项信息。
- 码：（列）能够唯一标识实体的属性，就像表中的列，也可以是多个列搭配在一起起到唯一标识的作用。
- 候选码：唯一标识字段的数据，这张数据可能有多个，比如id和账号都是唯一的。
- 主码：从候选码中选出的一个码，用于唯一标识每个元组。比如从id和账号中选择了账号作为主码。（就是主键）
- 外码：引用了另外一个关系中的主码的属性，建立了两个关系之间的联系。（就是外键）
- 主属性：候选码中出现的属性。
- 非主属性：不包含在任何候选码中的属性。
...
- **实体**：通常是现实世界的业务对象（逻辑对象也可以）。在 ER 图中，实体使用矩形框表示。
- **属性**：即某个实体拥有的属性。在 ER 图中，属性使用椭圆形表示。
- **联系**：即实体与实体之间的关系，在 ER 图中用菱形表示
...
如下表，进货、销售存在二次分割，则不满足第一范式

![incorrectNF1](https://raw.githubusercontent.com/ZXJ-OvO/picgo-img/master/202308101049453.png)

如下表满足第一范式

![correctNF1](https://raw.githubusercontent.com/ZXJ-OvO/picgo-img/master/202308101050299.png)
...
![dfdc86b0e2babe1f8da03d8e2b17ad06_720w](https://raw.githubusercontent.com/ZXJ-OvO/picgo-img/master/202308101627743.webp)

第二范式：

![46b18d25eb2a0da487e4f1b4520c979f_720w](https://raw.githubusercontent.com/ZXJ-OvO/picgo-img/master/202308101628411.webp)

![e3d91cd4a91ae656c9bf874ae298836a_720w](https://raw.githubusercontent.com/ZXJ-OvO/picgo-img/master/202308101628814.png)

- 函数依赖（functional dependency）：**若在一张表中，在属性（或属性组）X的值确定的情况下，必定能确定属性Y的值，那么就可以说Y函数依赖于X，写作 X → Y**。也就是说，在数据表中，不存在任意两条记录，它们在X属性（或属性组）上的值相同，而在Y属性上的值不同。这也就是“函数依赖”名字的由来，类似于函数关系 y = f(x)，在x的值确定的情况下，y的值一定是确定的。

  > 例如，姓名函数依赖于学号，写作 **学号 → 姓名**。但是反过来，因为可能出现同名的学生，所以有可能不同的两条学生记录，它们在姓名上的值相同，但对应的学号不同，所以我们不能说学号函数依赖于姓名。
  >
  > 例如其他的函数依赖关系：
  >
  > - 系名 → 系主任
...
- 传递函数依赖：
  ...
  假如 Z 函数依赖于 Y，且 Y 函数依赖于 X （严格来说还有一个X 不包含于Y，且 Y 不函数依赖于Z的前提条件），那么我们就称 Z 传递函数依赖于 X ，记作
  $$
  X \xrightarrow{T} Z
  $$
...
![image-20230810180556632](https://raw.githubusercontent.com/ZXJ-OvO/picgo-img/master/202308101805863.png)

> 具体问题具体分析，如果只是单体项目，不涉及分库分表，并发不高的情况下，外键带来的好处（数据一致性、级联操作更加方便）还是值得考虑的。
...
- 存储过程可以看成是一些 SQL 语句的集合，中间增加了一些逻辑控制语句以应对比较复杂的业务逻辑开发。
...
|      | delete                                                       | drop                                         | tuncate                                  |
| ---- | ------------------------------------------------------------ | -------------------------------------------- | ---------------------------------------- |
| 用法 | 只删除数据                                                   | 删除整个表                                   | 只清空数据，清空后自增从1重新开始        |
| 归属 | DML<br />操作会放到 rollback segment 中，事务提交之后才生效。 | DDL<br />操作立即生效                        | DDL<br />操作立即生效                    |
| 效率 | 执行产生binlog日志，可以回滚，所以效率最低                   | 直接释放表占用的所有空间，无法撤销，效率最高 | 不产生日志，并将自增值和索引恢复到初始值 |
...
- 大部分关系型数据库都使用 SQL 来操作数据库中的数据。并且，大部分关系型数据库都支持事务的 ACID 四大特性。
...
- MySQL 是一种开源中小型的关系型数据库，主要用于持久化存储我们的系统中的一些数据。



#### MySQL有什么优点

1. 成熟稳定、开源免费、开箱即用、社区活跃
2. 支持分库分表、读写分离、高可用、事务支持优秀
3. 兼容性优秀
...
![summary-of-mysql-field-types](https://raw.githubusercontent.com/ZXJ-OvO/picgo-img/master/202308112020812.png)
...
| 特性     | VARCHAR                                                    | CHAR                                                   |
| -------- | ---------------------------------------------------------- | ------------------------------------------------------ |
| 存储消耗 | 变长字符串，根据存储的实际数据长度占用空间，节省存储空间。 | 固定长度字符串，始终占用固定空间，不浪费存储空间。     |
| 隐式填充 | 额外保存长度信息在行数据中，不会自动填充空格               | 没有额外的长度信息，但是会填充空格到指定长度           |
| 存储范围 | 0 ~ 65535                                                  | 0 ~ 255                                                |
| 索引长度 | 使用存储的实际数据长度计算索引长度。                       | 始终使用指定的长度计算索引长度，不受存储数据长度影响。 |
| 读写速度 | 较低                                                       | 较快                                                   |
...
- varchar (100) 可以满足更大范围的字符存储需求，有更好的业务拓展性。而 varchar(10)存储超过 10 个字符时，就需要修改表结构。
...
| 有无时区信息 | 无                                        | 有                                        |
| 内存占用     | 4字节                                     | 8字节                                     |
| 表示范围     | 1000-01-01 00:00:00 ~ 9999-12-31 23:59:59 | 1970-01-01 00:00:01 ~ 2037-12-31 23:59:59 |
...
|        | null                   | ''（空字符串）        |
| ------ | ---------------------- | --------------------- |
| 确定性 | 不确定的值             | 表示一个长度为0的空值 |
| 长度   | 占用空间               | 0（不占用空间）       |
| 查询   | 只能使用 is [not] null | 可以使用比较运算符    |
...
> 1. COUNT(*)：用于计算满足条件的记录总数，无论字段是否包含NULL值。NULL值会被计入COUNT的结果中。
> 2. COUNT(column)：COUNT(column)用于计算指定列中非NULL值的数量。只会统计非NULL值的数量。
...
![13526879-3037b144ed09eb88](https://raw.githubusercontent.com/ZXJ-OvO/picgo-img/master/202308112122865.png)
...
| 查询缓存       | 执行查询语句的时候，会先查询缓存（MySQL 8.0 版本后移除，因为不实用） |
| 分析器         | 没有命中缓存的话，SQL 语句就会经过分析器，分析器先预检SQL目的再检查语法是否正确 |
| 优化器         | 按照 MySQL 认为最优的方案去执行                              |
| 执行器         | 执行语句，然后从存储引擎返回数据<br />执行语句之前会先判断是否有权限，如果没有权限的话，就会报错 |
...
>
> `select version();`：查看当前的MySQL版本
>
> `show variables like '%storage_engine%'`：查看当前默认的索引引擎
>
> ------
>
> 默认存储引擎
...
>
> - MySQL 5.5.5 之后：InnoDB

| 引擎               | 支持 | 说明                                                         | 事务 | XA   | Savepoints |
| ------------------ | ---- | ------------------------------------------------------------ | ---- | ---- | ---------- |
| InnoDB             | 默认 | 支持事务、行级锁和外键，默认引擎                             | 是   | 是   | 是         |
| MRG_MYISAM         | 是   | 将多个表联合成一个表使用，在超大规模数据存储时很有用         | 否   | 否   | 否         |
| MEMORY             | 是   | 基于哈希的内存存储引擎，拥有极高的插入，更新和查询效率。但是会占用和数据量成正比的内存空间。只在内存上保存数据，意味着数据可能会丢失 | 否   | 否   | 否         |
| BLACKHOLE          | 是   | /dev/null 存储引擎（向其中写入的数据会消失）                 | 否   | 否   | 否         |
| MyISAM             | 是   | 高速引擎，拥有较高的插入，查询速度，但不支持事务             | 否   | 否   | 否         |
| CSV                | 是   | 基于 CSV 格式文件存储数据(应用于跨平台的数据交换)            | 否   | 否   | 否         |
| ARCHIVE            | 是   | 将数据压缩后进行存储，非常适合存储大量的独立的，作为历史记录的数据，但是只能进行插入和查询操作 | 否   | 否   | 否         |
| PERFORMANCE_SCHEMA | 是   | Performance Schema                                           | 否   | 否   | 否         |
| FEDERATED          | 否   | Federated MySQL 存储引擎                                     | NULL | NULL | NULL       |

> XA协议：是分布式事务处理中的一种协议标准，用于协调和管理跨多个资源管理器（如数据库）的事务。XA协议定义了在分布式环境中执行事务的一致性和隔离级别。
>
> Savepoints：Savepoints是在数据库事务中使用的一种机制，用于将事务分割成更小的逻辑单元，并在需要时进行部分回滚。Savepoints允许在事务执行过程中创建一个标记点，以便在后续的操作中可以回滚到该标记点之前的状态。
...
- **存储引擎是基于表的，而不是数据库。**我们甚至可以为不同的数据库表设置不同的存储引擎以适应不同场景的需要。
...
|                        | InnoDB                                                       | MyISAM                             |
| ---------------------- | ------------------------------------------------------------ | ---------------------------------- |
| 是否支持行级锁         | 默认支持                                                     | 只有表级锁，所以并发性能极低       |
| 是否支持事务           | 支持，实现了SQL定义的四个隔离界别<br />支持提交和回滚事务。<br />默认使用可重读隔离级别解决了幻读问题发生 | 不支持                             |
| 是否支持外键           | 支持                                                         | 不支持                             |
| 是否支持崩溃后安全恢复 | 支持，重启恢复到崩溃前的状态，这个过程依赖于`redo log`       | 不支持                             |
| 是否支持MVCC           | 支持（MVCC相当于行级锁的升级）                               | 不支持                             |
| 索引实现方式           | 基于B+树，数据文件本身就是索引文件                           | 基于B+树，数据文件和索引文件分离   |
| 性能                   | 性能随核心数量增加而线性增长                                 | 读写不能并发，处理能力和核心数无关 |

![InnoDB 和 MyISAM 性能对比](https://raw.githubusercontent.com/ZXJ-OvO/picgo-img/master/202308130942234.png)



#### MyISAM 和 InnoDB 如何选择？

结合两种引擎的特点和缺陷，并引用《MySQL 高性能》的如下文字作答：
...
数据库事务可以保证多条SQL构成一个逻辑上的整体。这个整体遵循：**要么全部执行成功,要么全部不执行** 。
...
![数据库事务示意图](https://raw.githubusercontent.com/ZXJ-OvO/picgo-img/master/202308130940245.png)



关系型数据库都有AIDC特性：

1. **原子性**（`Atomicity`）：事务是最小的执行单位，不允许分割。该特性保证动作要么全部完成，要么全部不执行。
2. **隔离性**（`Isolation`）：并发访问数据库时，一个用户的事务不被其他事务所干扰，并发事务之间数据库是独立的；
3. **持久性**（`Durability`）：一个事务被提交之后。它对数据库中数据的改变是持久的，即使数据库发生故障也不应该对其有任何影响。
4. **一致性**（`Consistency`）：执行事务前后，数据保持一致。
...
![concurrency-consistency-issues-dirty-reading-ee15b0b9](https://raw.githubusercontent.com/ZXJ-OvO/picgo-img/master/202308121042685.png)
...
![concurrency-consistency-issues-missing-modifications-5b2e2bd8](https://raw.githubusercontent.com/ZXJ-OvO/picgo-img/master/202308121043757.png)
...
![concurrency-consistency-issues-unrepeatable-read-ff9186e1](https://raw.githubusercontent.com/ZXJ-OvO/picgo-img/master/202308121044415.png)
...
![concurrency-consistency-issues-phantom-read-7cba7a85](https://raw.githubusercontent.com/ZXJ-OvO/picgo-img/master/202308121044881.png)
...
|          | 不可重复读                                                   | 幻读                                                         |
| -------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 重点倾向 | 数据被修改、数据减少                                         | 数据增加                                                     |
| 解决方式 | 执行 `delete`、`update` 操作的时候，可以直接对记录加锁，保证事务安全 | 执行 `insert`时，由于记录锁只能锁住已经存在的记录，为了避免插入新记录，需要依赖间隙锁。也就是说执行 `insert` 操作的时候需要依赖 临建锁 进行加锁来保证不出现幻读 |

> 🌈
>
> 记录锁：Record Lock，又称行锁
>
> 间隙锁：Gap Lock
>
...
|          | 锁             | MVCC（多版本并发控制MVCC，Multiversion concurrency control） |
| -------- | -------------- | ------------------------------------------------------------ |
| 模式     | 悲观控制模式   | 乐观控制模式                                                 |
| 实现手段 | 共享锁、排他锁 | 隐藏字段、read view、undo log                                |
...
- **共享锁（S 锁）**：
  ...
  - 允许多个事务同时获取（锁兼容）
    ...
    > - 读写锁可以做到`读读并行`，但是无法做到 `写读`、`写写并行`。
    > - 另外，根据锁粒度的不同，又被分为 **表级锁(table-level locking)** 和 **行级锁(row-level locking)** 。
...
- **排他锁（X 锁）**：
  ...
  - 不允许多个事务同时获取。如果一个记录已经被加了排他锁，其他事务不能再对这条记录加任何类型的锁（锁不兼容）
...
|    serializable（可串行化）    | 所有的事务依次逐个执行，这样事务之间就完全不可能产生干扰     |  ×   |     ×      |  ×   |
...
serializable隔离级别是通过锁来实现的，read-committed 和 repeatable-read 隔离级别是基于 MVCC 实现的。不过，serializable 之外的其他隔离级别可能也需要用到锁机制，就比如 repeatable-read 在当前读情况下需要使用加锁读来保证不会出现幻读。
...
可以将上述内容转换为Markdown表格如下所示：

| 锁类型 | 特点                                                         | 存储引擎支持                                 |
| ------ | ------------------------------------------------------------ | -------------------------------------------- |
| 表级锁 | - 粒度大<br>- 针对 非索引字段 加的锁<br>- 锁住整张表<br>- 实现简单，资源消耗较少<br>- 加锁快，不会出现死锁<br>- 触发锁冲突的概率高<br>- 高并发下效率极低 | MyISAM、InnoDB                               |
| 行级锁 | - 粒度小<br>- 针对 索引字段 加的锁<br>- 锁住当前操作行<br>- 开销大，加锁慢，可能触发死锁，但是并发度高 | 行级锁和存储引擎有关，是在存储引擎层面实现的 |



#### 行级锁的使用有什么注意事项？

- InnoDB 的行锁是针对索引字段加的锁，当执行 `UPDATE`、`DELETE` 语句时，如果 `WHERE`条件中字段没有命中唯一索引或者索引失效的话，就会导致扫描全表对表中的所有行记录进行加锁。

- 即使用了索引也有可能会全表扫描，这是由于 MySQL 优化器的原因。
...
**在 InnoDB 默认的隔离级别 可重复读 下，行锁默认使用的是 临建锁。但是，如果操作的索引是唯一索引或主键，InnoDB 会对 临建锁 进行优化，将其降级为 Record Lock，即仅锁住索引本身，而不是范围。**



#### 共享锁和排他锁呢？

不论是表级锁还是行级锁，都存在共享锁、排他锁 这两类：
...
可以直接存储文件对应的二进制数据即可。不过会严重影响数据库性能。推荐使用云对象存储，或者基于Spring Boot 整合 MinIO 自建分布式文件服务。
...
没有特殊要求（即 InnoDB 无法满足的功能如：列存储，存储空间数据等）的情况下，所有表必须使用 InnoDB 存储引擎 InnoDB 支持事务，支持行级锁，更好的恢复性，高并发下性能更好。
